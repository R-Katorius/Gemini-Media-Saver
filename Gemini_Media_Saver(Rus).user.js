// ==UserScript==
// @name         Gemini Media Saver (Rus)
// @namespace    http://tampermonkey.net/
// @version      2.5
// @description  Добавлено: сохранение состояния кнопок после перезагрузки страницы (localStorage)
// @author       Gemini AI
...
(function() {
    ...
    function processElements() {
        ...
        chatImages.forEach((img) => {
            ...
            // Используем URL или alt-текст как уникальный ключ для фото
            ...
            if (existingBtn) {
                if (isSaved && existingBtn.querySelector('span').innerText !== 'Готово') {
                    existingBtn.querySelector('span').innerText = 'Готово';
            ...
            imgBtn.innerHTML = `${iconPhotoSVG} <span>${isSaved ? 'Готово' : 'Сохранить фото'}</span>`;
            ...
            imgBtn.onclick = (e) => {
                ...
                persistData(); // Сохраняем в localStorage
                imgBtn.querySelector('span').innerText = 'Готово';
        ...
        responses.forEach((resp) => {
            ...
            // Генерируем ключ на основе первых 50 символов текста, чтобы он был уникальным для ответа
            ...
            if (existingTxtBtn) {
                if (isSaved && existingTxtBtn.querySelector('span').innerText !== 'Сохранено') {
                    existingTxtBtn.querySelector('span').innerText = 'Сохранено';
            ...
            txtBtn.innerHTML = `${iconTextSVG} <span>${isSaved ? 'Сохранено' : 'Сохранить текст'}</span>`;
            ...
            txtBtn.onclick = (e) => {
                ...
                const finalOutput = textTarget.innerText.replace('Сохранить текст', '').replace('Сохранено', '').trim();
                ...
                txtBtn.querySelector('span').innerText = 'Сохранено';
